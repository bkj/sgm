...
# This generates graphs in dense-matrix format, which is super inefficient
# We can save them as sparse edgelists, though
...
for(n in all_num_nodes) {
    ...
    n_perm = length((num_seeds+1):nrow(x$B)) 
    perm   = sample(n_perm)
    x$B[(num_seeds+1):nrow(x$B),] = x$B[(num_seeds+1):nrow(x$B),][perm,]
    x$B[,(num_seeds+1):nrow(x$B)] = x$B[,(num_seeds+1):nrow(x$B)][,perm]
